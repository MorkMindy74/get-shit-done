...
// ─── Command Registry (table-driven dispatch with lazy requires) ─────────────
...
const COMMANDS = {
  'state': (args, raw, cwd) => {
    ...
    if (subcommand === 'update') {
      state.cmdStateUpdate(cwd, args[2], args[3], raw);
  ...
  'resolve-model': (args, raw, cwd) => {
    const commands = require('./lib/commands.cjs');
    commands.cmdResolveModel(cwd, args[1], raw);
  },

  'find-phase': (args, raw, cwd) => {
  ...
  'template': (args, raw, cwd) => {
    ...
    if (subcommand === 'select') {
      ...
      if (fieldsIdx !== -1) {
        try {
          fields = JSON.parse(args[fieldsIdx + 1]);
        } catch (e) {
          error(`Invalid JSON for --fields: ${e.message}`);
        }
      }
  ...
  'generate-slug': (args, raw, _cwd) => {
    const commands = require('./lib/commands.cjs');
    commands.cmdGenerateSlug(args[1], raw);
  },

  'current-timestamp': (args, raw, _cwd) => {
    const commands = require('./lib/commands.cjs');
    commands.cmdCurrentTimestamp(args[1] || 'full', raw);
  },

  'list-todos': (args, raw, cwd) => {
    const commands = require('./lib/commands.cjs');
    commands.cmdListTodos(cwd, args[1], raw);
  },

  'verify-path-exists': (args, raw, cwd) => {
    const commands = require('./lib/commands.cjs');
    commands.cmdVerifyPathExists(cwd, args[1], raw);
  },

  'config-ensure-section': (args, raw, cwd) => {
    const config = require('./lib/config.cjs');
    config.cmdConfigEnsureSection(cwd, raw);
  },

  'config-set': (args, raw, cwd) => {
    const config = require('./lib/config.cjs');
    config.cmdConfigSet(cwd, args[1], args[2], raw);
  },

  'config-get': (args, raw, cwd) => {
    const config = require('./lib/config.cjs');
    config.cmdConfigGet(cwd, args[1], raw);
  },

  'history-digest': (args, raw, cwd) => {
    const commands = require('./lib/commands.cjs');
  ...
  'init': (args, raw, cwd) => {
    ...
    const INIT_WORKFLOWS = {
      'execute-phase': () => init.cmdInitExecutePhase(cwd, args[2], raw),
      'plan-phase': () => init.cmdInitPlanPhase(cwd, args[2], raw),
    ...
    const workflowHandler = INIT_WORKFLOWS[workflow];
    if (workflowHandler) {
      workflowHandler();
    } else {
  ...
  'phase-plan-index': (args, raw, cwd) => {
    const phase = require('./lib/phase.cjs');
    phase.cmdPhasePlanIndex(cwd, args[1], raw);
  },

  'state-snapshot': (args, raw, cwd) => {
    const state = require('./lib/state.cjs');
    state.cmdStateSnapshot(cwd, raw);
  },

  'summary-extract': (args, raw, cwd) => {
    const commands = require('./lib/commands.cjs');
    const summaryPath = args[1];
